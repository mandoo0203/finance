class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id?].present?
      ...
      if stock
        @user_stock = UserStock.new(stock_id: stock, user:current_user)
      else
        stock = Stock.look_up_stock(params[:stock_ticker])
        if stock.save
          @user_stock = UserStock.new(stock_id: stock, user:current_user)
        else
          @usser_stock = nil
          flash[:error] = "Stock is Unavailable"
        end
      end
    ...
    respond_to do |format|
      if @user_stock.save
        format.html { redirect_to my_portfolio_path, notice: 'Stock #{@user_stock.stock.ticker} stock was added' }
